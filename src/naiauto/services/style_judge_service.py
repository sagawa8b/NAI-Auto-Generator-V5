...
from ..core.arena.models import ArenaState, Combo
...
class StyleJudgeService:
    ...
    def run(
        ...
        for i, combo in enumerate(combos, start=1):
            if cancelled():
                on_event(JudgeFinished(completed=completed, failed=failed, stopped=True))
                return
            ...
            try:
                ...
                on_event(
                    JudgeFinished(completed=completed, failed=failed, error=str(e), error_key="Exception")
                )
                return
        ...
        on_event(JudgeFinished(completed=completed, failed=failed))
...
def judge_results_to_csv(state: ArenaState, use_prefix: bool = True) -> str:
    ...
    writer.writerow(JUDGE_CSV_HEADER)
    for rank, combo in enumerate(ranked, start=1):
        writer.writerow(
            [rank, combo.judge_score, format_artist_block(combo.slots, use_prefix), combo.judge_reason]
        )
