...
변경점:
  ...
  - WebP: EXIF UserComment에 save_raw_png()가 넣어 둔 텍스트 필드를 그대로 복원해
    PNG의 tEXt 청크와 같은 모양(_parse_text_fields)으로 넘긴다. 그것도 없으면
    PNG와 똑같이 stealth로 내려간다 — 무손실 WebP는 알파 LSB를 그대로 보존한다.
...
def read_metadata(source: str | Path | bytes) -> dict[str, Any] | None:
    ...
    with img:
        ...
        # 숨김 메타데이터는 형식을 가리지 않는다 — 알파 LSB에 실려 있으므로 무손실이면
        # PNG든 WebP든 그대로 살아남는다. 웹을 거치며 텍스트 청크만 털린 이미지에서는
        # 이쪽이 유일하게 남은 출처다 (NovelAI의 이미지 확인 페이지도 이걸 읽는다).
        stealth_raw = _read_stealth(img)
